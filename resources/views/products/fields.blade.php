...
<div style="flex: 50%;max-width: 50%;padding: 0 4px;" class="column">
    ...
    @else
        ...
        <div class="form-group row ">
            {!! Form::label('market_id', trans("lang.product_market_id"),['class' => 'col-3 control-label text-right']) !!}
            <div class="col-9"> 
                {!! Form::select('market_id[]', $market, null, ['class' => 'select2 form-control','multiple' =>'multiple']) !!}
                <div class="form-text text-muted">{{ trans("lang.product_market_id_help") }}</div>
            </div>
        </div>
    @endif
    ...
    @if($data['mode'] == 1)
    <div class="form-group row ">
        {!! Form::label('copy_market_id', 'Copy Market',['class' => 'col-3 control-label text-right']) !!}
        <div class="col-9"> 
            <select class="select2 form-control" id="copy_market_id" name="copy_market_id">
                <option value="">Select If Copy Market</option>
                @foreach($market as $key=>$item)
                <option value="{{ $key }}">{{ $item }}</option>
                @endforeach
            </select>    
            <!--{!! Form::select('copy_market_id', $market, null, ['class' => 'select2 form-control']) !!}-->
            <div class="form-text text-muted">{{ trans("lang.product_market_id_help") }}</div>
        </div>
    </div>
    @endif
